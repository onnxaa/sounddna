...
void MorphEngine::AddPoint(const DNAProfile& profile, double position, double blend) {
  MorphPoint pt;
  pt.profile = profile;
...
DNAProfile MorphEngine::GetCurrentMorph() const {
  if (mMorphPoints.empty()) return DNAProfile{};
...
DNAProfile MorphEngine::InterpolateProfiles(const DNAProfile& a,
                                             const DNAProfile& b,
                                             double t) const {
  DNAProfile result;
