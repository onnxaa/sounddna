...
constexpr int kMaxProfileLayers = 8;
...
struct GenoProfile {
  ...
  static GenoProfile FromFeatureVector(const std::vector<double>& vec);
  double SimilarityTo(const GenoProfile& other) const;
  GenoProfile InterpolateWith(const GenoProfile& other, double t) const;
};

struct GenoTransferParams {
  std::array<double, static_cast<int>(GenoGene::Count)> amounts = {};
  std::array<bool, static_cast<int>(GenoGene::Count)> locks = {};
