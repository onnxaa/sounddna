...
std::vector<double> GenoProfile::ToFeatureVector() const {
  ...
  vec.push_back(spectral.pitch);
  vec.push_back(spectral.pitchConfidence);
  ...
  vec.push_back(dynamics.attackMs);
  vec.push_back(dynamics.releaseMs);
  vec.push_back(dynamics.compressionRatio);
  vec.push_back(dynamics.envelopeMean);

  vec.push_back(stereo.width);
  vec.push_back(stereo.phaseCorrelation);
  vec.push_back(stereo.balance);
  vec.push_back(stereo.phaseDrift);

  vec.push_back(noise.noiseFloorDb);
  for (int i = 0; i < kNumNoiseCoefs; ++i)
    vec.push_back(noise.noiseShape[i]);
  vec.push_back(noise.signalToNoise);
  vec.push_back(noise.humContent);
  vec.push_back(noise.spectralTilt);

  vec.push_back(texture.saturationAmount);
  vec.push_back(texture.harmonicDistortion);
  vec.push_back(texture.analogWarmth);
  vec.push_back(texture.tapeSaturation);
  vec.push_back(texture.transientShape);

  return vec;
}

GenoProfile GenoProfile::FromFeatureVector(const std::vector<double>& vec) {
  GenoProfile profile;
  size_t pos = 0;
  ...
  profile.spectral.pitch = read();
  profile.spectral.pitchConfidence = read();

  profile.dynamics.rms = read();
  profile.dynamics.peak = read();
  profile.dynamics.crestFactor = read();
  profile.dynamics.dynamicRange = read();
  profile.dynamics.attackMs = read();
  profile.dynamics.releaseMs = read();
  profile.dynamics.compressionRatio = read();
  profile.dynamics.envelopeMean = read();

  profile.stereo.width = read();
  profile.stereo.phaseCorrelation = read();
  profile.stereo.balance = read();
  profile.stereo.phaseDrift = read();

  profile.noise.noiseFloorDb = read();
  for (int i = 0; i < kNumNoiseCoefs && pos < vec.size(); ++i)
    profile.noise.noiseShape[i] = vec[pos++];
  profile.noise.signalToNoise = read();
  profile.noise.humContent = read();
  profile.noise.spectralTilt = read();

  profile.texture.saturationAmount = read();
  profile.texture.harmonicDistortion = read();
  profile.texture.analogWarmth = read();
  profile.texture.tapeSaturation = read();
  profile.texture.transientShape = read();

  return profile;
}

double GenoProfile::SimilarityTo(const GenoProfile& other) const {
...
GenoProfile GenoProfile::InterpolateWith(const GenoProfile& other, double t) const {
  GenoProfile result;
