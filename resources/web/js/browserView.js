class DNABrowserUI {
  ...
  buildPanel() {
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;border-bottom:1px solid var(--border)">
        ...
        <button class="btn btn-small" onclick="GENO.browser.hide()">Close</button>
      </div>
      <div style="padding:8px 12px;border-bottom:1px solid var(--border)">
        <input type="text" id="browserSearch" placeholder="Search by instrument, genre, character..."
               ...
               oninput="GENO.browser.search(this.value)">
      </div>
  ...
  search(query) {
    GENO.Bridge.sendMessage(8, btoa(JSON.stringify({ query, filters: this.filters })));
  ...
  selectProfile(profile) {
    GENO.Bridge.sendMessage(9, btoa(JSON.stringify(profile)));
